...
function addBoxToCell(box, grid, x, y) {
    grid['x' + x + 'y' + y + 'box' + box.i] = true;
}

function removeBoxFromCell(box, grid, x, y) {
	delete grid['x' + x + 'y' + y + 'box' + box.i];
}
...
function checkCollisions() {
    collisions = {};
	let maxBox = 0;
	Object.keys(grid).forEach(function(element) {
		if (element.split('box')[1] > maxBox) maxBox = element.split('box')[1];
	});
    for (let x = 0; x < gridW; x++) {
        for (let y = 0; y < gridH; y++) {
			for (let b1 = 1; b1 <= maxBox; b1++) {
				if (grid['x' + x + 'y' + y + 'box' + b1]) {
					for (let b2 = b1; b2 <= maxBox; b2++) {
						if (grid['x' + x + 'y' + y + 'box' + b2]) addCollision(b1, b2);
					}
				}
			}
        }
